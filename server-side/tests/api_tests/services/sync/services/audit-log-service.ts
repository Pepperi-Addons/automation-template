...
export class AuditLogService {
    ...
    async getFieldsFromSchemasInAudit(resultObject:any,schemaNames:any){
        let data : any = {}
        let fields = resultObject.ResourcesData.map(resource =>{
            let target = Object.entries(schemaNames).find(([key, value]) => value === resource.Schema.Name)
            if(target){
                data[target[0]] = resource.Objects
            }
        })
        return data
    }
